...
malloc(size_t size)
{
	...
	/* 0 바이트 요청에 대해서는 NULL 포인터를 반환합니다. */
	if (size == 0)
		return NULL;

	/* Find the smallest descriptor that satisfies a SIZE-byte
		 request. */
	/* SIZE 바이트 요청을 충족시키는 가장 작은 디스크립터를 찾습니다. */
	...
	if (d == descs + desc_cnt)
	{
		...
		/* SIZE가 모든 디스크립터보다 큽니다.
           SIZE와 아레나 크기의 합만큼의 페이지를 할당합니다. */
		...
		/* Initialize the arena to indicate a big block of PAGE_CNT
			 pages, and return it. */
		/* 아레나를 초기화하여 PAGE_CNT 페이지의 큰 블록을 나타내고 반환합니다. */
		a->magic = ARENA_MAGIC;
		a->desc = NULL;
		a->free_cnt = page_cnt;
		return a + 1;

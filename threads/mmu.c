...
/* PML4에 사용자 가상 페이지 UPAGE와 커널 가상 주소 KPAGE가 식별하는 물리 프레임 간의 매핑을 추가합니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다. KPAGE는 사용자 풀에서 palloc_get_page()로 얻은 페이지여야 합니다.
 * WRITABLE이 true인 경우, 새 페이지는 읽기/쓰기가 가능합니다; 그렇지 않으면 읽기 전용입니다.
...
{
	/* upage와 kpage가 페이지 정렬이 맞는지, upage가 사용자 주소 공간에 속하는지 확인합니다. */
	ASSERT(pg_ofs(upage) == 0);
	ASSERT(pg_ofs(kpage) == 0);
	ASSERT(is_user_vaddr(upage));
	ASSERT(pml4 != base_pml4);

	/* upage에 대한 페이지 테이블 항목(pte)을 조회하거나 생성합니다. */
	uint64_t *pte = pml4e_walk(pml4, (uint64_t)upage, 1);

	/* pte가 존재하면, 이를 kpage의 물리 주소로 설정하고, 페이지 속성을 설정합니다. */
	if (pte)
		*pte = vtop(kpage) | PTE_P | (rw ? PTE_W : 0) | PTE_U;
	return pte != NULL; /* pte가 NULL이 아니면 true를 반환합니다. */
}

 ...
 * Returns -1 on fail. */
void argument_stack(char **argv, int cnt, struct intr_frame *_if)
{
	int i = 0;
	// 1. argv 값 뒤에서부터 푸쉬
	// 2. padding
...
/* 파일 FILE의 오프셋 OFS부터 시작하는 세그먼트를 UPAGE 주소에 로드합니다.
 * 총 READ_BYTES + ZERO_BYTES 바이트의 가상 메모리가 다음과 같이 초기화됩니다:
 *
 * - UPAGE에서 READ_BYTES 바이트는 FILE에서 오프셋 OFS부터 읽어야 합니다.
 *
						 ...
						 uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
	...
	/* read_bytes 또는 zero_bytes가 남아있는 동안 반복합니다. */
	while (read_bytes > 0 || zero_bytes > 0)
	{
		/* Do calculate how to fill this page.
		 * We will read PAGE_READ_BYTES bytes from FILE
		...
		/* Advance. */
		/* read_bytes, zero_bytes, upage를 업데이트하며 다음 페이지로 진행합니다. */
		read_bytes -= page_read_bytes;
		zero_bytes -= page_zero_bytes;
		upage += PGSIZE;
